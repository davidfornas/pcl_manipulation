/*
 * Copy a pointCloud2 msg to disk filtering to remove outliers.
 *
...
void cloud_cb (const sensor_msgs::PointCloud2 input)
{
  ...
  std::cerr << "PointCloud after filtering: " << cloud_f2->width * cloud_f2->height 
       << " data points (" << pcl::getFieldsList (*cloud_f2) << ").";
  //http://pointclouds.org/documentation/tutorials/reading_pcd.php#reading-pcd
...
int main (int argc, char** argv) {
  ...
  vPtr = VirtualImagePtr(new VirtualImage(nh,"stereo/left/image_rect", "stereo/left/camera_info", 1));
  vPtr->open(Ic) ;
  vPtr->acquire(Ic);
